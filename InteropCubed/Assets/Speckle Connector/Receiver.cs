...
namespace Speckle.ConnectorUnity
{
    ...
    public class Receiver : MonoBehaviour
    {
        ...
        public void ReceiveAnnotation()
        {
            ...
            Task.Run(async () =>
            {
                try
                {
                    ...
                    var commit = mainBranch.commits.items[0];
                    GetBaseObject(commit.referencedObject, commit.id);
                }
                catch (Exception e)
                {
                    throw new SpeckleException(e.Message, e, true, SentryLevel.Error);
        ...
        private async void GetBaseObject(string objectId, string commitId)
        {
            try
            {
                ...
                Dispatcher.Instance().Enqueue(() =>
                {
                    ...
                    var rc = GetComponent<RecursiveConverter>();
                    var go = rc.ConvertRecursivelyBaseToNative(@base, commitId);
